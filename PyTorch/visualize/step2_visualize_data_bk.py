...
class MotionPlayer:
    ...
    def __init__(self, model, data, dataset, show_trajectory=False, past_frames=10, future_frames=45):
        self.model = model
        self.data = data
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Visualize past and future trajectories"
    )
    parser.add_argument(
        "--past-frames",
        type=int,
    ...
    with mujoco.viewer.launch_passive(model, data, key_callback=key_callback) as viewer:
        while viewer.is_running():
            ...
            if player.show_trajectory:
                assert hasattr(player, 'past_traj') and hasattr(player, 'future_traj'), \
                    "Trajectory data not available for visualization."
                with viewer.lock():
                    viewer.user_scn.ngeom = 0
                    
                    # --- PREPARE DATA (Contiguous Float64 is Critical) ---
                    # Positions
                    traj_p_pos = np.ascontiguousarray(player.past_traj, dtype=np.float64)
                    traj_f_pos = np.ascontiguousarray(player.future_traj, dtype=np.float64)
                    # Orientations
                    traj_p_ori = np.ascontiguousarray(player.past_orient, dtype=np.float64)
                    traj_f_ori = np.ascontiguousarray(player.future_orient, dtype=np.float64)

                    # --- DRAW PAST (Blue) ---
                    if len(traj_p_pos) > 0:
                        draw_trajectory(viewer, traj_p_pos, traj_p_ori, color=[0.2, 0.5, 1.0, 1.0])

                    # --- DRAW FUTURE (Red) ---
                    if len(traj_f_pos) > 0:
                        draw_trajectory(viewer, traj_f_pos, traj_f_ori, color=[1.0, 0.2, 0.2, 1.0])
